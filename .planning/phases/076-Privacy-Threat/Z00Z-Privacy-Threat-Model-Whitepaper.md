...
- [Z00Z Main Whitepaper](../../../docs/Z00Z-Main-Whitepaper.md)
- [Z00Z OnionNet Whitepaper](../../../docs/Z00Z-OnionNet-Whitepaper.md)
- [Z00Z Linked Liability Whitepaper](Z00Z-Linked-Liability-Whitepaper.md)
- [Z00Z Agentic Offline Economy Whitepaper](../../../docs/Z00Z-Agentic-Offline-Economy-Whitepaper.md)
- [Z00Z Cross-Chain Integration Whitepaper](../../../docs/Z00Z-Cross-Chain-Integration-Whitepaper.md)
- [Z00Z Roadmap Blueprint](../../../docs/tech-papers/Z00Z-Roadmap-Blueprint.md)
- [Z00Z Corpus Terminology And Abbreviations Reference](../../../docs/Z00Z-Corpus-Terminology-Reference.md)
...
### 6.4 Cover Traffic And Shape Discipline

Some of the most useful privacy controls are deliberately mundane: fixed or
bucketed publication slots, bounded random delay, batch padding, dummy
envelopes, and fixed-size encrypted fields. These controls make timing, count,
and size observations less decisive without changing the settlement theorem.

The privacy model should treat those controls as wallet, transport, and
aggregator discipline rather than as automatic anonymity. Dummy or cover
traffic must be domain-isolated, economically neutral, and auditable enough not
to create fake value. Padding should hide operational shape without becoming a
second transaction species that marks the user as privacy-seeking. Jitter should
be bounded so it weakens correlation without breaking deadlines, force-inclusion
paths, or liability challenge windows.

This discipline is especially relevant for ACK-to-publication flows, PTB or
batching lanes, and low-load OnionNet periods. Without shape discipline, a
confidential object can still leak through a distinctive delay, a rare batch
size, a repeated output count, or a one-to-one ACK-to-spend timing pattern.
...
### 7.4 Mode And Recovery UX Boundaries

Mode and recovery design are privacy surfaces. A wallet that supports personal,
corporate, test, recovery, or penalized states should bind those states into the
signed domain or mode context and make the active state impossible to miss in
the signing UX. A user should not be able to accidentally produce a corporate
or audit-facing artifact while believing they are in an ordinary private flow.

Recovery should also be treated as a bounded disclosure path. Social recovery,
time-locked recovery, hardware backup, or identity-rotation flows may be useful,
but they must not silently clear an active liability lock, erase a penalty case,
or expose unrelated wallet history. A recovery flow can restore access to a
domain only under its own challenge window and policy checks; it is not a
shortcut around `Selective Reveal`, `Exculpability`, or penalty settlement.
...
- `Traffic-shape and cover tests`: fixed-size or bucketed batch behavior,
  dummy-envelope neutrality, bounded jitter, and low-load transport warnings.
- `Mode and recovery tests`: signed mode binding, clear UX state, recovery
  challenge windows, and the rule that recovery cannot bypass active liability
  or penalty state.
...
| Future `FraudProof` and `FraudProofVerifier` | Target-architecture signatures from the linked-liability paper include `check_conflict(candidate, settled_set, policy)` and `verify_liability_guilt(domain_commitment, fraud_proof, policy_id)`. | These signatures remain incomplete until their `LeakageContract` declares `fraud_outputs`, audience scope, and exculpability evidence without exposing unrelated wallet state. |
