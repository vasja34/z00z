...
This paper owns the liability object model, fraud-case activation, bond, lock,
penalty, and unlock semantics. The companion [Z00Z Privacy Threat Model And Metrics](Z00Z-Privacy-Threat-Model-Whitepaper.md) owns the broader privacy axes,
disclosure vocabulary, telemetry boundaries, wallet UX privacy risks, transport
non-claims, and leakage-contract rules that constrain any future `FraudProof`
implementation.
...
#### Acknowledgement And Deadline Evidence

Some offline rights need an intermediate acknowledgement surface before final publication. A merchant or peer may receive a signed acknowledgement of intended outputs, a deadline, and enough encrypted recovery material to verify that the promised outputs are actually theirs. In that family of flows, fraud evidence should cover three distinct cases: the published output set differs from the signed acknowledgement, the relevant nullifier never appears before the deadline, or the same input or right is presented through incompatible receipts.

This evidence must be available when the dispute is raised. If a proof depends on an off-chain acknowledgement signature, the protocol or wallet layer should provide a durable anchor such as a commitment to the acknowledgement, a watchtower copy, a counterparty receipt, or a mandatory reference inside the settlement package. A liability design that requires evidence but leaves it easy to lose creates a punishment story that only works for lucky counterparties.

Output binding must also include the fields that determine spendability, not only nominal value. A signed output set should bind commitment, context identifier, encrypted blinding material, output type, and any policy fields needed to reconstruct or spend the received right. Changing a `context_id` while keeping the apparent value is still a fraud case if it makes the counterparty's promised right unusable.
...
Challenge paths therefore need their own abuse resistance. A proof submission surface should be proof-first, bounded by a validity window, deduplicated by the challenged object and proof root, and bonded or otherwise rate-limited so fake challenges cost more than they consume. If the system later accepts pre-transaction batches, each batch should bind `state_root_in`, `state_root_out`, and a conflict set covering nullifiers, context identifiers, and commitment identifiers so parallel batches cannot apply stale or overlapping state.

Liveness fallback belongs in the service and settlement-adjacent layer rather than in the liability proof itself. Force-inclusion transactions, watchdog reports, emergency modes, and aggregator or validator slashing can keep delayed-settlement flows alive under censorship or aggregator silence, but they should remain clearly separated from the narrower proof that a liability domain actually committed fraud.
...
Hardware tokens and secure elements can reduce accidental or malware-driven reuse by keeping keys non-exportable, enforcing user presence, maintaining local spend tags, or requiring a policy co-signer. They do not replace the settlement-level nullifier, proof, and liability logic. A token can refuse to sign twice under its own policy, but if the holder controls or defeats the device, final double-spend handling still depends on later evidence and the linked-liability path.
...
### 10.4 Cross-Phase Compatibility With 067-072

Linked Liability must compose with the already planned quorum, checkpoint,
proof, rollup, inbox, and offline-transaction authorities. Phase 075 adds the
fraud and liability layer; it must not silently redefine any upstream authority
surface.

| Phase | Required compatibility rule for Linked Liability |
| --- | --- |
| 067 Sharded Consensus | A `FraudProof` may rely on quorum-bound publication evidence only after the package, route, placement, theorem input, DA publication, and quorum certificate bind the same committed subject. Aggregator quorum evidence must not become wallet ownership proof, checkpoint validity, recursive proof validity, or liability guilt by itself. |
| 068 Checkpoint Contract | Liability evidence must be retained through the checkpoint contract lanes: `CheckpointExecInput`, `CheckpointArtifact`, `CheckpointLink`, `CheckpointArchiveManifestV1`, `CheckpointDaReferenceV1`, and `CheckpointPublicationEvidenceV1`. DA publication readiness may start the challenge or dispute clock, but it does not prove fraud or settlement validity by itself. |
| 069 Recursive Proof | Recursive sidecars and future proof backends may compress or audit checkpoint evidence only over the same checkpoint statement. They must not delete raw packages, exact proof bytes, witness data, archive manifests, or liability evidence that remains needed for fraud, challenge, compensation, or unlock review. |
| 070 Rollup Node | The rollup publication layer must provide provider-neutral DA/archive adapters, retrieval evidence, bounded challenge classes, and checkpoint theorem evidence. Provider receipts, CIDs, blob hashes, SDK receipts, watcher or operator observations, and publication observations are support evidence, not liability guilt or settlement validity by themselves. |
| 071 Request-Bound Inbox | Request-bound inbox records, route buckets, helper hints, and remote scan worker material are advisory and privacy-sensitive. They may support evidence discovery or ordering, but they must not mutate wallet state, claim ownership, open a liability case, or persist binding-rich receiver records as fraud authority. |
| 072 Offline Transaction | `TxPackage` remains the canonical offline transaction node, and any future `OfflineTxBundleV1` remains a wrapper around canonical packages. Linked Liability may consume package conflict surfaces, duplicate-spend evidence, ACK/deadline evidence, and bundle conflict sets, but it must not introduce a second offline transaction family or bypass `verify_full_tx_package(...)`, wallet import gates, storage-owned apply, or theorem-bound checkpoint validation. |

The cross-phase rule is intentionally asymmetric. Phases 067-072 provide
ordered publication, retained evidence, checkpoint truth, recursive audit
material, advisory inbox hints, and offline package conflict surfaces. Phase 075
adds the proof-triggered liability interpretation and enforcement lifecycle on
top of those surfaces. It must not move ownership of package validity, wallet
ownership, checkpoint validity, DA readiness, or privacy telemetry into the
liability layer.

#### Fraud Flow Closure Status

The whitepaper-level flow is conceptually complete, but the executable flow is
not closed until the objects below are implemented and traced to the upstream
phase authorities.

| Flow step | Current description in this paper | Required executable closure |
| --- | --- | --- |
| Fraud evidence appears | Conflicting receipts, packages, nullifiers, duplicate claims, ACK mismatch, missed deadline, or other policy breach reach settlement-time comparison. | Define a canonical `FraudEvidenceRecordV1` or equivalent record that stores the conflict source, checkpoint reference, archive reference, request/ACK anchor when used, and privacy leakage contract. |
| Fraud proof is built | `FraudProof` extracts the bounded liability reveal and proves common origin without exposing unrelated wallet history. | Define `FraudProofV1`, canonical bytes, verifier API, reject reasons, exculpability proof, and `leakage_contract_ref` aligned with the Privacy Threat Model. |
| Proof is validated | Settlement verifier or proof builder verifies the conflict set and opens liability handling only after proof acceptance. | Bind `FraudProofVerifier` to checkpoint artifacts, package theorem evidence, archive material, and proof-specific policy. False, incomplete, stale, wrong-domain, or over-revealing proofs must fail closed. |
| Liability state is written | A `LockRegistry` entry activates the affected `LiabilityDomain`, case status, bond state, victim set, scope, and unlock condition. | Define `LockRegistryEntryV1`, writer owner, durable storage location, idempotent duplicate behavior, replay rules, and checkpoint or storage references. |
| Future rights are blocked | New use from the locked domain is rejected or quarantined until resolution. | Wire wallet package creation, package import, local acceptance, storage apply, or checkpoint verification to consult the lock state without freezing unrelated domains. |
| Bond and penalty are applied | `BondRef` is frozen or slashed, victims are compensated, and `PenaltyPolicy` controls the response. | Define `BondDebitRecordV1`, `PenaltySettlementRecordV1`, `CompensationReceiptV1`, ordering rules, partial or failed payment behavior, and exact validation of penalty satisfaction. |
| Challenge and finality windows run | Challenge paths should be proof-first, window-bounded, deduplicated, and bonded or rate-limited. | Map liability challenge windows onto checkpoint publication readiness, rollup challenge classes, retention gates, and abuse-resistant challenge submission rules. |
| Unlock or retirement occurs | Case resolves after penalty payment, victim compensation, dispute closure, cooldown, proof invalidation, or domain retirement. | Define unlock proof or resolution records and state transitions from locked or compensating to unlock-ready, released, or retired. Recovery must not bypass an active liability case. |

This table is the implementation handoff boundary. Anything in the middle
column is specification intent. Anything in the right column remains open work
until backed by code, tests, retention evidence, and GSD closeout artifacts.
...
| `leakage_contract_ref` | Declares the audience-bounded fields revealed by this proof | Must align with the Privacy Threat Model `LeakageContract` and name `fraud_outputs` explicitly |
