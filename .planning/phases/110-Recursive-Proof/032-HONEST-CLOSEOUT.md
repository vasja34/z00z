...
1. Claim packages bind the accepted claim statement to the authoritative source-root contract consumed by downstream verification, rather than leaving the source root outside the signed tuple.
...
- A broader `z00z_simulator` release-style suite completed successfully with `--release --features test-fast --features wallet_debug_dump`.
- Supporting long-running-suite evidence remains recorded in [reports/full_verify-report-long-running-tests.txt](/home/vadim/Projects/z00z/reports/full_verify-report-long-running-tests.txt). That report is supporting evidence only; it does not replace the required bootstrap-first and release-style command order above.
