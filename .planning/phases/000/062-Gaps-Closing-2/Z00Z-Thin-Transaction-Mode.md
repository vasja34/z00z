...
| JMT design | canonical `SettlementPath`, proof-envelope discipline, path-index non-authority, storage-owned leaf presence and deletion | a helper-side index snapshot that points at canonical paths and proof envelopes without becoming semantic truth |
...
- a replacement for `SettlementPath`;
- a public semantic root parallel to `SettlementStateRoot`;
...
flowchart TD
  ...
  Entry --> Path["Canonical SettlementPath family
...
| `SettlementPath` | `definition_id`, `serial_id`, `terminal_id` | Index entries must resolve back onto the canonical settlement-path family or an explicit compatibility bridge; they must not replace it with generic note terminology. |
| `SettlementStateRoot` | 32-byte live settlement-state root | A signed index root is not a `SettlementStateRoot`, `CheckRoot`, or generalized settlement root. |
...
- `SettlementStateRoot` is the live semantic settlement root.
- `SettlementPath` is the live public path family; historical `AssetPath`
  wording must not be revived as current protocol truth.
...
| `index_id -> note_id / note / Merkle path` as the protocol signature | Use `entry_id` plus the `SettlementPath` family and proof-envelope references. Generic note terms are not current Z00Z corpus signatures. |
