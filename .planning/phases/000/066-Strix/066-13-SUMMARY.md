...
workflow now points to `./scripts/run_pentest_tools.sh` instead of presenting
...
- Canonical entrypoint and generic prompt path
  - `pentest-local.prompt.md` now names
    `./scripts/run_pentest_tools.sh` as the only external human or agent
    entrypoint.
...
- Z00Z adapter routing
  ...
  - The prompt now includes a canonical Z00Z run example that reuses
    `./scripts/run_pentest_tools.sh --profile z00z` and keeps heavy closeout on
...
- `rg -n './scripts/run_pentest_tools.sh|z00z-pentest-profile|attack-surfaces-create|z00z-crypto-auditor|gsd-audit-4.prompt.md' .github/prompts/pentest-*.prompt.md`
