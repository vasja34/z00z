...
| `WS-10` | 066-10 | 6 | `REQ-017` | `T-066-06` | `./scripts/run_pentest_tools.sh --docker-sandbox` remains the canonical pentest Docker entrypoint | e2e | `python3 -m unittest tests.penetration.test_packaging_portability tests.penetration.test_docker_scope` | ✅ | ✅ green |
| `WS-10` | 066-10 | 6 | `REQ-018` | `T-066-06` | Docker pentests run from `pack_z00z_project.sh` tarballs, not mutable live checkout state | e2e | `./pack_z00z_project.sh --output /tmp/z00z-pentest-validate.tar.gz && ./scripts/run_pentest_tools.sh --docker-sandbox --archive /tmp/z00z-pentest-validate.tar.gz --mode quick --static-only --check-only` | ✅ | ✅ green |
...
- `python3 -m unittest discover tests/penetration` completed green with `68` passing tests.
- `bash -n scripts/penetration/*.sh tools/penetration/docker/run_pentest_container.sh scripts/run_pentest_tools.sh pack_z00z_project.sh unpack_z00z_project.sh` completed green.
- `python3 -m py_compile scripts/penetration/*.py tests/penetration/*.py` completed green.
- `python3 scripts/penetration/validate_scope.py scripts/penetration/scope.yaml --denylist scripts/penetration/denied-tools.txt --json` completed green with `status=OK`, `hosts=2`, `urls=2`, and `scope_path=scripts/penetration/scope.yaml`.
- `bash scripts/penetration/check_pentest_tools.sh --json --strict` completed green and remained truthful with `present: 15`, `missing: 0`, `broken: 0`, and `missing_required: 0`.
