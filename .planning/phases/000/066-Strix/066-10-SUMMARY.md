...
`./scripts/run_pentest_tools.sh`. The lane adds an archive-driven Docker wrapper,
...
- Canonical portable entrypoint
  - Added `./scripts/run_pentest_tools.sh` as the human and agent entrypoint for
...
- `./scripts/run_pentest_tools.sh --mode quick --static-only --check-only --artifact-dir /tmp/z00z-pentest-local-check`
...
- `./scripts/run_pentest_tools.sh --docker-sandbox --archive /tmp/z00z-pentest-plan-check.tar.gz --mode quick --static-only --check-only`
...
- The supplied-archive and auto-pack Docker journeys both completed green
  through `./scripts/run_pentest_tools.sh`.
