...
- Validation snapshot recorded while refreshing this spec on `2026-07-03`:
  - `python3 -m unittest discover tests/penetration` passed (`68` tests)
  - `python3 -m pytest tests/penetration` is currently blocked in this
    workspace (`No module named pytest`)
  - `bash -n scripts/penetration/*.sh tools/penetration/docker/run_pentest_container.sh scripts/run_pentest_tools.sh pack_z00z_project.sh unpack_z00z_project.sh` passed
  - `python3 -m py_compile scripts/penetration/*.py tests/penetration/*.py` passed
  - `python3 scripts/penetration/validate_scope.py scripts/penetration/scope.yaml --denylist scripts/penetration/denied-tools.txt --json` passed
  - `bash scripts/penetration/check_pentest_tools.sh --json --strict` passed
    (`present=15`, `missing=0`, `broken=0`, `missing_required=0`;
    all default Phase 066 tools are installed and the current host snapshot has
    no missing or broken tool entries)
...
- `scripts/penetration/run_source_sast.sh`
  - live seam for deterministic AST target derivation, structural source-SAST
    execution, secrets-lane artifacts, and deep-mode missing-tool semantics
...
- `./scripts/run_pentest_tools.sh`
  - canonical human and agent entrypoint for local and Docker pentest
    flows
- `./scripts/run_pentest_tools.sh --docker-sandbox --archive <tarball>`
  - supplied-archive Docker journey
- `./scripts/run_pentest_tools.sh --docker-sandbox`
...
- `./scripts/run_pentest_tools.sh` generic quick static-only run;
- `./scripts/run_pentest_tools.sh --docker-sandbox` supplied-archive run;
- `./scripts/run_pentest_tools.sh --docker-sandbox` auto-pack run;
...
- `tests/penetration/test_source_sast_runner.py`
...
| --- | --- | --- | --- |
| `U-066-01` | `tests/penetration/test_scope_validation.py` | existing | Owns the canonical `scripts/penetration/{scope.yaml,denied-tools.txt}` contract and should stay isolated from broader runner behavior |
...
| `U-066-02A` | `tests/penetration/test_source_sast_runner.py` | existing | Correct seam for `run_source_sast.sh`, deterministic AST target derivation, secrets artifacts, and deep-mode structural-tool semantics |
...
flowchart TD
  ...
  B -->|accepted| C[run_local_pentest.sh or scripts/run_pentest_tools.sh]
  ...
  G --> H[reports/z00z-pentests-report-timestamp]
...
| `U-066-02A` | `tests/penetration/test_source_sast_runner.py` | Prove source-aware SAST runner behavior through the public CLI | AST targets are derived deterministically into `raw/ast/sg-targets.txt`; `sast/summary.json` and normalized status files reflect `semgrep`, `sg`, `tree-sitter`, `trufflehog`, and secrets lanes truthfully; deep mode upgrades `tree-sitter` from skipped to required structural coverage | Structural-tool absence recorded as success, missing deep-mode `tree-sitter` marked skipped instead of missing, secrets outputs absent, AST targets omit canonical Phase 066 entrypoints |
...
| --- | --- | --- | --- |
| `U-066-01`, `E2E-066-02`, `E2E-066-03` | scope YAML variants, denied-tool list | `validate_scope.py` exists and is callable | `tests/penetration/fixtures/scope/` plus `scripts/penetration/denied-tools.txt` |
...
| `U-066-02A` | normalized scope JSON, tool-status fixtures, synthetic source-SAST tool binaries | `run_source_sast.sh` exists and is callable; temporary workspace can expose fake `semgrep`, `sg`, `tree-sitter`, `gitleaks`, `trufflehog`, and `trivy` binaries | synthesized inside `tests/penetration/test_source_sast_runner.py` temporary directories |
...
| `U-066-02A` | source-aware SAST lane completes with truthful structural-tool status | `raw/ast/sg-targets.txt`, `raw/ast/tree-sitter.txt` when available, `sast/summary.json`, `normalized/sast.{sg,tree-sitter,trufflehog}.status.json`, and raw secrets outputs | canonical Phase 066 entrypoints appear in AST targets; deep mode marks missing `tree-sitter` as `missing`, not `skipped` |
...
| `E2E-066-08` / `E2E-066-09` | Docker path consumes archive and exports results | host-mounted `.security-artifacts/<timestamp>/` and `reports/z00z-pentests-report-<timestamp>/` | visible container output and host-side report copy-back |
...
**Demonstrates**
The canonical local workflow can start from `./scripts/run_pentest_tools.sh` or
...
**Setup**
- Use a temporary workspace fixture with a valid localhost-only `scripts/penetration/scope.yaml`.
...
- `reports/z00z-pentests-report-<timestamp>/` exists for the same timestamp.
...
### `U-066-02A` Source-Aware SAST Runner Structural And Secrets Coverage

**Demonstrates**
The public `scripts/penetration/run_source_sast.sh` seam derives deterministic
AST targets from the normalized scope, records structural-tool status
truthfully, produces secrets-lane artifacts, and tightens deep-mode structural
coverage without inventing a parallel runner path.
...
**Setup**
- Synthesize a temporary normalized scope JSON and tool-status manifest.
- Expose fake `semgrep`, `sg`, `tree-sitter`, `gitleaks`, `trufflehog`, and
  `trivy` binaries through the test workspace.
- Run the public CLI in both `standard` and `deep` modes.

**Assertions**
- `raw/ast/sg-targets.txt` deterministically includes
  `scripts/penetration/run_local_pentest.sh`,
  `scripts/run_pentest_tools.sh`, and
  `scripts/penetration/run_source_sast.sh`.
- `sast/summary.json` returns `completed` when all exercised structural tools
  are available and `completed-with-missing-tools` when optional structural
  tools are absent.
- `normalized/sast.sg.status.json`,
  `normalized/sast.tree-sitter.status.json`, and
  `normalized/sast.trufflehog.status.json` preserve truthful status values.
- Raw secrets artifacts exist at
  `raw/secrets/sast.{trufflehog,gitleaks,trivy}.json`.
- In `deep` mode, missing `tree-sitter` is recorded as `missing` rather than
  `skipped`.

**Must fail if**
- canonical Phase 066 entrypoints disappear from the AST target set;
- secrets lanes do not emit raw artifacts;
- deep mode silently skips missing `tree-sitter`;
- structural-tool status is upgraded to success without evidence.
...
python3 scripts/penetration/validate_scope.py scripts/penetration/scope.yaml --denylist scripts/penetration/denied-tools.txt --json
bash scripts/penetration/check_pentest_tools.sh --json
python3 -m unittest tests.penetration.test_source_sast_runner
test -L .codex/skills
test -L .codex/agents
...
rg -n "hydra|john|hashcat|medusa|patator|metasploit|msfvenom|pacu|responder|netexec|crackmapexec|commix|tplmap" scripts/penetration .github/skills/pentest-*/SKILL.md .github/prompts/pentest-*.prompt.md scripts/penetration/denied-tools.txt
rg -n "MCP|HexStrike server|Strix runtime|LLM_API_KEY|hydra|john|hashcat|medusa|patator|metasploit|msfvenom|responder|netexec|crackmapexec|commix|tplmap" scripts/penetration .github/skills/pentest-*/SKILL.md .github/prompts/pentest-*.prompt.md scripts/penetration/denied-tools.txt
...
| `WS-02` | `U-066-02`, `U-066-02A` | `E2E-066-01`, `E2E-066-07`, `E2E-066-10` | canonical `tools/penetration/` root, truthful tool status, source-SAST structural coverage, no formal-verification bleed |
...
| `WS-06` | `U-066-02A`, `U-066-03`, `U-066-04` | `E2E-066-01`, `E2E-066-03`, `E2E-066-04` | runner orchestration, source-SAST lane status, exit preservation, artifact production |
...
| `WS-12` | `U-066-01`, `U-066-02`, `U-066-02A`, `U-066-03`, `U-066-04` | `E2E-066-03`, `E2E-066-04` | regression coverage, source-SAST missing-tool semantics, skip artifacts, redaction, rejection paths |
...
- Repository-surface availability is no longer a blocker. The live Phase 066
  ...
  `./scripts/run_pentest_tools.sh`, `.github/skills/pentest-*`,
