...
canonical human and agent entrypoint is `./scripts/run_pentest_tools.sh`; reusable
...
`reports/z00z-pentests-report-<timestamp>/` for human-facing reports.
...
- `scripts/penetration/run_local_pentest.sh` is the local orchestrator invoked
  by skills and `./scripts/run_pentest_tools.sh`.
...
- `./scripts/run_pentest_tools.sh --docker-sandbox` is the canonical Docker
