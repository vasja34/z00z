...
- `tests/penetration/test_source_sast_runner.py` now closes the
  previously missing source-aware SAST runner seam, deterministic AST target
  derivation, and deep-mode structural-tool status coverage.
...
| `TT-066-03` | Implement `test_tool_manifest.py`, `test_source_sast_runner.py`, and checker contract assertions | `TT-066-01` | Tool root stays under `tools/penetration/`; missing tools are recorded truthfully; `run_source_sast.sh` derives deterministic AST targets and truthful deep-mode structural-tool status; no formal-verification tool root is used |
...
| --- | --- | --- |
| `tests/penetration/test_scope_validation.py` | `WS-01`, `WS-08`, `WS-12` | `validate_scope.py`, `scripts/penetration/scope.yaml`, `scripts/penetration/denied-tools.txt` |
...
| `tests/penetration/test_source_sast_runner.py` | `WS-02`, `WS-06`, `WS-12` | `run_source_sast.sh`, normalized scope JSON, tool-status manifests, source-SAST raw and normalized artifacts |
...
| `tests/penetration/test_packaging_portability.py` | `WS-10` | `pack_z00z_project.sh`, `scripts/run_pentest_tools.sh` |
...
### Source-Aware SAST Fixtures

- normalized scope JSON fixtures are synthesized inside
  `test_source_sast_runner.py`
- fake `semgrep`, `sg`, `tree-sitter`, `gitleaks`, `trufflehog`, and `trivy`
  binaries are synthesized inside the test workspace
- assertions bind to `raw/ast/sg-targets.txt`, `sast/summary.json`,
  normalized source-SAST status files, and raw secrets outputs
