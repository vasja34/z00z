...
```markdown
Read ~/.copilot/agents/gsd-framework-selector.md for instructions.
...
Copy template:
```bash
cp "$HOME/.copilot/get-shit-done/templates/AI-SPEC.md" "${PHASE_DIR}/${PADDED_PHASE}-AI-SPEC.md"
```
...
```markdown
Read ~/.copilot/agents/gsd-ai-researcher.md for instructions.
...
```markdown
Read ~/.copilot/agents/gsd-domain-researcher.md for instructions.
...
```markdown
Read ~/.copilot/agents/gsd-eval-planner.md for instructions.
