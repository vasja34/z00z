...
Always validate with these flags when relevant:

```bash
cargo test --release 
```

Use smaller targeted commands while iterating if useful, but do not finish without the required validation scope unless blocked.
