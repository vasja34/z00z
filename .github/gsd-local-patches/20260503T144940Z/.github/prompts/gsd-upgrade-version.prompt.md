...
argument-hint: 'target_ref=main [source_repo=https://github.com/gsd-build/get-shit-done] [runtime=--copilot] [live_root=.github/get-shit-done]'
---
...
- `source_repo`: `${input:source_repo:https://github.com/gsd-build/get-shit-done}`
- `runtime`: `${input:runtime:--copilot}`
- `live_root`: `${input:live_root:.github/get-shit-done}`
...
- map upstream `get-shit-done/`, `skills/`, and `agents/` content into the installed Copilot layout before deciding whether a file is missing, renamed, or drifted;
...
- create any backup copies for existing `txt`, `md`, `json`, `yaml`, `yml`, and `csv` files that will be fully replaced during the reinstall inside `.github/gsd-local-patches/` instead of next to the live files under `.github/get-shit-done/`;
...
- `.github/skills/gsd-add-tests/SKILL.md` still preserves the intended local behavior from the saved patch set (advisory-only or hard-gate), without silently reverting that behavior.
...
- `.github/get-shit-done/bin/lib/config.cjs`

## Required Upgrade Workflow

1. Inspect the current live GSD tree under `live_root`.
2. Resolve `target_ref` explicitly and verify that the requested branch or tag exists before replacing anything.
3. Map the chosen upstream source into the local Copilot install layout and compare like-for-like installed paths.
...
7. Perform the upgrade using source or tagged manual install logic with the runtime flag.
8. Verify the live installed version directly from `./.github/get-shit-done/VERSION` after install instead of relying only on installer output.
...
11. Explicitly call out the `gsd-add-tests` local changes, the plan-end auto-fix behavior, and the phase-level auto-fix behavior as mandatory review items.
...
17. Surface any installer warnings that materially affect Copilot usage, including unreplaced `.claude` path references.
...
- `.github/skills/gsd-add-tests/SKILL.md` still matches the intended local behavior from the selected patch set (advisory-only or hard-gate);
...
- the live `./.github/get-shit-done/VERSION` file matches the requested installed version when `target_ref` is a concrete release tag;
...
- the final live version read from `./.github/get-shit-done/VERSION`;
...
- required backup copies for full overwrite of existing text-like files were created inside `.github/gsd-local-patches/` instead of next to the live `.github/get-shit-done/` files;
...
- Do not create sibling `.bak` files inside `.github/get-shit-done/` during this upgrade flow.
...
- Confirm Copilot-specific local anchors (`gsd-add-tests` and plan-end auto-fix behavior) are explicitly included in restore review.
