...
2. **Validate output:**
   Check that .graphify/graph.json exists and is valid JSON with nodes[] and edges[] arrays.
...
3. **Copy artifacts to .planning/graphs/:**
   ```
   cp .graphify/graph.json .planning/graphs/graph.json
   cp .graphify/graph.html .planning/graphs/graph.html
   cp .graphify/GRAPH_REPORT.md .planning/graphs/GRAPH_REPORT.md
   ```
