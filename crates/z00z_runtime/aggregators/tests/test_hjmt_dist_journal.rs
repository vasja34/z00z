...
fn test_handles_partition_replay() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let secondary = AggregatorId::new(8);
    ...
    let mut sim =
        DistSim::new(route, [primary, secondary]).expect("dist sim must build for lawful members");
    sim.seed(primary, record.clone())
        .expect("primary seed must accept the live recovery record");
    sim.partition(secondary)
        .expect("partition must accept a known secondary");
    sim.enqueue_delayed(JournalFrame::new(primary, secondary, 1, record.clone()), 1);
    ...
    sim.heal(secondary)
        .expect("heal must accept a known secondary");
    ...
    .expect("healed secondary must resume after replicated recovery applies");

    sim.enqueue_replay(JournalFrame::new(primary, secondary, 1, record.clone()), 0);
    let replay = sim.step();
    assert_eq!(replay.len(), 1);
    assert_eq!(replay[0].stage, FrameStage::ReplayIgnored);
    assert_eq!(
        sim.node(secondary).expect("secondary node").applied_count(),
        1,
        "replay must stay idempotent on the secondary state"
    );
...
fn test_catchup_fails_closed() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let secondary = AggregatorId::new(28);
    let ready = SecondaryState::ready(secondary);
    ...
    sim.enqueue_delayed(
        JournalFrame::new(primary, secondary, 3, latest_record.clone()),
        2,
    );
    sim.enqueue_front(JournalFrame::new(primary, secondary, 2, stale_record));

    let first = sim.step();
    assert_eq!(first.len(), 1);
    assert_eq!(first[0].stage, FrameStage::Applied);
    let stale_err = sim
        .sync_verdict(secondary, &latest_record)
        .expect_err("stale secondary state must fail before the later frame lands");
    ...
    sim.sync_verdict(secondary, &latest_record)
    ...
    .expect("caught-up secondary must resume after the latest state transfer");
