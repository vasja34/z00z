...
fn out_dir() -> PathBuf {
    managed_test_output_root("e2e14")
}
...
fn eq_guard(a: [u8; 32], b: [u8; 32]) -> Result<(), &'static str> {
    if a == b {
        Ok(())
    } else {
        Err("mismatch")
...
#[test]
fn test_stage4_drift() {
    if cfg!(debug_assertions) {
        return;
    }

    let rows_fix = vec_set();
    let mut rep = String::from("E2E-14 drift matrix\n");
    ...
    for row in &rows_fix {
        ...
        let rid_drift = hash_zk::<ReceiverIdDomain>("RID", &[&seed[..]]);
        let view_drift = hash_to_scalar_zk::<ViewKeyDomain>("VIEW", &[&seed[..]])
            .expect("view drift")
            .to_bytes();
        ...
        assert_ne!(rid_wallet, rid_drift, "RID drift must differ on {}", row.id);
        assert_ne!(
            view_wallet, view_drift,
            "VIEW drift must differ on {}",
            row.id
        );

        assert!(
            eq_guard(rid_wallet, rid_drift).is_err(),
            "RID negative control must fail on {}",
            row.id
        );
        assert!(
            eq_guard(view_wallet, view_drift).is_err(),
            "VIEW negative control must fail on {}",
            row.id
        );

        let rid_eq = rid_wallet == rid_drift;
        let view_eq = view_wallet == view_drift;

        rep.push_str(&format!(
            "id={} rid_eq={} view_eq={} rid_drift={} view_drift={}\n",
            row.id,
            if rid_eq { 1 } else { 0 },
            if view_eq { 1 } else { 0 },
            hex::encode(rid_drift),
            hex::encode(view_drift)
        ));
        rows.push(serde_json::json!({
            ...
            "rid_drift": hex::encode(rid_drift),
            "view_base": row.view,
            "view_wallet": hex::encode(view_wallet),
            "view_drift": hex::encode(view_drift),
            "rid_equal": rid_eq,
            "view_equal": view_eq
        }));
    }

    create_dir_all(out_dir()).expect("mkdir outputs/tests/e2e14");
    write_file(out_dir().join("e2e14_drift_matrix.txt"), rep.as_bytes()).expect("write matrix");
    ...
    write_file(out_dir().join("e2e14_vectors.json"), &vec_bytes).expect("write vectors");
}
