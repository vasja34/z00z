use super::wallet_service_core::ReceiverLabelList;
use super::wallet_service_reachability::ReachabilityWallet;

fn warn_output_dir_create(err: &z00z_utils::io::IoError) {
    z00z_utils::logger::Logger::warn(
        &z00z_utils::logger::TracingLogger,
        &format!("Failed to create wallets output directory: {err}"),
    );
}

fn warn_runtime_config_fallback(setting: &str, err: &crate::WalletError) {
    z00z_utils::logger::Logger::warn(
        &z00z_utils::logger::TracingLogger,
        &format!("Failed to load {setting} from wallet_config.yaml: {err}"),
    );
}

impl WalletService {
    fn startup_auto_lock_policy() -> AutoLockPolicy {
        crate::services::wallet_runtime_config::resolve_auto_lock_policy()
            .unwrap_or_else(|err| {
                warn_runtime_config_fallback("wallet auto-lock policy", &err);
                AutoLockPolicy::default()
            })
    }

    fn startup_receiver_cache_size() -> usize {
        crate::services::wallet_runtime_config::resolve_receiver_cache_size()
            .unwrap_or_else(|err| {
                warn_runtime_config_fallback("receiver cache size", &err);
                crate::receiver::DEFAULT_CACHE_SIZE
            })
    }

    fn startup_receiver_derive_rate_limit(
    ) -> Option<crate::services::wallet_runtime_config::ReceiverDeriveRateLimit> {
        crate::services::wallet_runtime_config::resolve_receiver_derive_rate_limit()
            .unwrap_or_else(|err| {
                warn_runtime_config_fallback("receiver derive rate limit", &err);
                None
            })
    }

    fn default_wallet_store(time_provider: Arc<dyn TimeProvider>) -> Arc<dyn WltStore> {
        Arc::new(crate::db::RedbWalletStore::new(
            time_provider,
            Arc::new(crate::db::Z00ZWalletIo),
        ))
    }
...
include!("wallet_session_construction_helpers.rs");
include!("wallet_session_construction_variants.rs");
