...
use crate::{
    ...
    extended_range_proof::{
        AggregatedPrivateStatement, AggregatedPublicStatement, ExtendedRangeProofService, ExtendedWitness, Statement,
    },
    range_proof::RangeProofService,
    ristretto::{
        pedersen::extended_commitment_factory::ExtendedPedersenCommitmentFactory, RistrettoPublicKey,
        RistrettoSecretKey,
    },
};
...
impl BulletproofsPlusService {
    ...
    pub fn init(
        ...
        Ok(Self {
            generators: RangeParameters::init(
                bit_length,
                aggregation_factor,
                BulletproofsPlusRistrettoPedersenGens {
...
impl BulletproofsPlusService {
    /// Construct a range proof using a provided RNG for deterministic proof generation.
    ///
    /// # ⚠️ Z00Z-Specific Extension (Non-Standard Tari API)
    ///
    /// **IMPORTANT**: This method is a Z00Z-specific extension to the Tari crypto library.
    /// It is NOT part of the original Tari codebase and should be maintained with extreme care.
    ///
    /// ## Rationale
    ///
    /// The original Tari `RangeProofService::construct_proof()` uses `OsRng` (system entropy),
    /// making range proofs non-deterministic. This causes issues for:
    /// - Genesis state generation (needs byte-for-byte reproducibility)
    /// - Golden snapshot testing (requires deterministic outputs)
    /// - Consensus-critical operations (must be deterministic across nodes)
    ///
    /// ## Implementation
    ///
    /// This method wraps `RistrettoRangeProof::prove_with_rng()` instead of `prove()`,
    /// allowing the caller to provide a deterministic RNG (e.g., `ChaChaRng` with fixed seed).
    ///
    /// ## Usage
    ///
    /// ```ignore
    ...
    /// let proof = service.construct_proof_with_rng(&key, value, &mut rng)?;
    /// ```
    ///
    /// ## Maintenance Notes
    ///
    /// - When updating Tari crypto dependencies, verify this method remains compatible
    /// - Do NOT remove this method without providing an alternative for deterministic proofs
    /// - If Tari adds native deterministic proof support, migrate to that instead
    /// - Document any changes to this method in the Z00Z changelog
    ///
    /// ## See Also
    ///
    /// - Original Tari API: `RangeProofService::construct_proof()`
    /// - Underlying function: `RistrettoRangeProof::prove_with_rng()`
    /// - Z00Z usage: `z00z_core::genesis::builder::construct_range_proof_with_rng()`
    pub fn construct_proof_with_rng<R: rand_core::CryptoRngCore>(
        &self,
        ...
        let proof = RistrettoRangeProof::prove_with_rng(
            &mut Transcript::new(self.transcript_label.as_bytes()),
            &statement,
            &witness,
            rng,
...
mod test {
    ...
    use crate::{
        commitment::{
            ExtendedHomomorphicCommitmentFactory, ExtensionDegree as CommitmentExtensionDegree,
        ...
        ristretto::{
            bulletproofs_plus::{
                BulletproofsPlusService, RistrettoAggregatedPrivateStatement, RistrettoAggregatedPublicStatement,
                RistrettoExtendedMask, RistrettoExtendedWitness, RistrettoStatement,
            },
