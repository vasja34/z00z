...
Application and protocol code must treat the crate as a three-lane facade:
...
3. `z00z_crypto::vendor::tari`: explicit opt-in lane for concrete Tari contracts.
...
### ✅ Explicit Backend Lane: `vendor::tari`

Use `z00z_crypto::vendor::tari` only when code must work with a concrete Tari contract such as a factory, signature type, or low-level proof service.

Current vendor exports from `src/vendor.rs`:

- Re-exported expert helpers:
  - `from_hex`, `to_hex`, `Hex`, `ByteArray`, `SafePassword`
  - `hash_domain`
  - `RistrettoPublicKey`, `RistrettoSecretKey`
  - `DerivedKeyDomain`, `DomainSeparatedHasher`, `DomainSeparation`
  - `PublicKeyTrait`, `SecretKeyTrait`
- Concrete Tari contracts:
...
Use the vendor lane only when a caller truly needs the Tari service or factory type.
...
Use `vendor::tari::DiffieHellmanSharedSecret` only if a caller explicitly needs the Tari type itself.
...
| Concrete Tari key types | ✅ Available | Re-exported for explicit backend users |
...
6. Keep the stable root Z00Z-owned; concrete Tari contracts belong in `vendor::tari`.
