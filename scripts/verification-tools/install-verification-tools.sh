...
Pin release downloads with env vars:
  ...
  The installer removes stale tools/formal_verification/versions.env copies
  instead of maintaining a second shadow file under the tool payload root.
...
Uninstall:
  Review current inventory with:
    ./scripts/verification-tools/install-verification-tools.sh --check --profile all --strict
...
cleanup_stale_versions_env_shadow() {
  mkdir -p "$TOOLS_DIR"
  rm -f "$TOOLS_DIR/versions.env"
}
...
self_test_repo_scripts() {
  ...
  if have shellcheck; then
    ...
    shellcheck -x -e SC1090,SC2016 \
      "$ROOT_DIR/scripts/verification-tools/install-verification-tools.sh" \
