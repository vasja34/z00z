...
```mermaid
flowchart TD
  Wallet[Wallet or SDK] --> Inner[Inner canonical envelope]
  Inner --> InnerAEAD[Encrypt to ingress recipient]
  InnerAEAD --> Onion[Sphinx-compatible transport wrapper]
  Onion --> Relay[Bridge, edge, and relay forwarding]
  Relay --> Exit[Exit unwraps transport only]
  Exit --> Opaque[Opaque inner ciphertext]
  Opaque --> Decrypt[Ingress decryptor]
  Decrypt --> WorkItem[Runtime WorkItem formation]
...
```mermaid
flowchart TD
  Floors[Privacy and diversity floors] --> Check[Evaluate live lane viability]
  Check -->|floors satisfied| Publish[Publish lane set]
  Check -->|floors missed| Contract[Contract live lanes]
  Contract --> Recheck[Recompute viable route diversity]
  Recheck -->|still safe| Publish
  Recheck -->|unsafe thin lanes| Delay[Delay admission or fail closed]
