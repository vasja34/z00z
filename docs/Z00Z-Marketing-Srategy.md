...
#### Consumer-Facing Message Layer

This strategy document is the **message and GTM translation** of the
consumer-facing map owned by the use-cases paper. It should compress that map
into repeatable public language rather than inventing a second retail taxonomy.
The job here is not to restate every family. The job is to make the most
legible consumer surfaces easy to explain without retail drift.

| Message pillar | User-level promise | Best proof surface | GTM caution |
| --- | --- | --- | --- |
| Private payments without a public social graph | Pay people or merchants without leaving a permanent public counterparty map | Litepaper plus use-cases offline-cash material | Do not overclaim total invisibility |
| Cash-like payment in weak connectivity | Local QR, NFC, or file handoff plus later reconciliation | Use-cases offline-cash flow and demos | Do not promise unconditional offline finality |
| Private subscriptions, vouchers, and merchant-scoped claims | Use bounded private claims instead of broad public allowances | Smart-cash paper plus use-cases policy family | Do not describe this as generic smart contracts |
| Private community distribution and coupons | Recipients and redemption graphs do not need to become public | Use-cases distribution family | Do not imply every aid or compliance workflow is already landed |
| Accountless paid access and session rights | Buy one bounded service session without a standing visible account relationship | Use-cases service-right examples plus smart-cash framing | Do not market this as a universal private VM |

This layer is retail-legible, but it does not change the launch order. Early
GTM can still start with qualified users, design partners, or integrators while
using these consumer-visible surfaces to make the category easier to understand.
...
```mermaid
flowchart TD
  A["Category clarity"] --> B["Trust and comprehension"]
  B --> C["Qualified reach"]
  C --> D["Measured activation"]
  D --> E["Retention and reuse"]
  E --> F["Ecosystem proof"]
  F --> G["Scaled distribution"]
  ...
  style E fill:#EDE7F6,stroke:#5E35B1,stroke-width:1px,color:#311B92
  style F fill:#ECEFF1,stroke:#546E7A,stroke-width:1px,color:#263238
  style G fill:#FFF8E1,stroke:#F9A825,stroke-width:1px,color:#5D4037
```
...
flowchart TD
  A["Phase 1
  Owned base and category clarity"] --> B["Phase 2
  Creator and partner amplification"]
  B --> C["Phase 3
  Measured pilot activation"]
  C --> D["Phase 4
  Retention proof and case studies"]
  D --> E["Phase 5
  Scaled paid and flagship distribution"]
